...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($userId!== null) {
        $sql = "INSERT INTO Shopping (UserId, ProductId, kolvo, address, time) VALUES ";
        foreach ($cartItems as $productId => $item) {
            $quantity = $item['quantity'];
            $sql.= "($userId, $productId, $quantity, '$address', '$time'),";
        }
        $sql = rtrim($sql, ',');
...
<body>
    <h2>покупка заказа</h2>


        <a href="buy.php">перейти к покупке</a>
        <a href="">отменить оформление</a>
    </form>
</body>
</html>
