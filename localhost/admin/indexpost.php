...
<a href="http://localhost/admin/formPost.php" style="background-color: gray; color: white; padding: 5px; border-radius: 5px;  text-decoration: none;">Добавить поставщика</a>
<a href="http://localhost/admin/formPrivoz.php" style="background-color: gray; color: white; padding: 5px; border-radius: 5px;  text-decoration: none;">Добавить привоз</a>
</div>
<?php
...
if($result = $conn->query($sql)){
    ...
    foreach($result as $row){
        echo "<tr>";
            echo "<td>" . $row["name"] . "</td>";
            echo "<td><a stylestyle='background-color: gray; color: white; padding: 5px; border-radius: 5px;  text-decoration: none;' href='updatePost.php?id=" . $row["id"] . "'>Изменить</a></td>";
...
if($result = $conn->query($sql)){
    ...
    foreach($result as $row){
        echo "<tr>";
            ...
            echo "<td>". $row["time"]. "</td>";
            echo "<td><a style='background-color: gray; color: white; padding: 5px; border-radius: 5px;  text-decoration: none;' href='updatePrivoz.php?id=". $row["id"]. "'>Изменить</a></td>";
            echo "<td><a href='podrobneePrivoz.php?id=". $row["id"]. "' style='background-color: gray; color: white; padding: 5px; border-radius: 5px;  text-decoration: none;'>Подробнее</a></td>";
